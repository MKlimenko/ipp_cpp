...
namespace ipp_cpp {
	...
	inline namespace ippdc {
		...
		template <typename T> auto MTFGetSize(int* pMTFStateSize) { static_assert(sizeof(T) == 0, "Unexpected overload"); }
		...
		template <typename T> auto BWTFwdGetBufSize_SelectSort(Ipp32u wndSize, Ipp32u* pBWTFwdBufSize, IppBWTSortAlgorithmHint sortAlgorithmHint) { static_assert(sizeof(T) == 0, "Unexpected overload"); }
